...
app.use((req, res, next) => {
  res.setHeader("X-TRACE-Build", "trace-v51-creator-ritual-archive-activity");
  next();
});
...
const ritualEnginePath = path.join(__dirname, "trace-creator-ritual-v1.js");
const ritualStylesPath = path.join(__dirname, "trace-creator-ritual-v1.css");
...
const CONFIG = Object.freeze({
  ...
  verificationEventWindowMs: envInt("TRACE_VERIFICATION_EVENT_WINDOW_MS", 6 * 60 * 60 * 1000, 60_000, 7 * 24 * 60 * 60 * 1000),
  verificationEventLimitPerIp: envInt("TRACE_VERIFICATION_EVENT_LIMIT_PER_IP", 120, 10, 5000),
});
...
app.get("/trace-creator-ritual-v1.js", (req, res) => {
  if (!fs.existsSync(ritualEnginePath)) return res.status(500).type("text/plain").send("TRACE creator ritual engine not found");
  res.setHeader("Cache-Control", "public, max-age=900, stale-while-revalidate=86400");
  res.setHeader("Content-Type", "application/javascript; charset=utf-8");
  return res.sendFile(ritualEnginePath);
});

app.get("/trace-creator-ritual-v1.css", (req, res) => {
  if (!fs.existsSync(ritualStylesPath)) return res.status(500).type("text/plain").send("TRACE creator ritual styles not found");
  res.setHeader("Cache-Control", "public, max-age=900, stale-while-revalidate=86400");
  res.setHeader("Content-Type", "text/css; charset=utf-8");
  return res.sendFile(ritualStylesPath);
});
...
const TRACE_GLYPH_STYLE_RULES = Object.freeze({
  ...
  quantum_lattice: Object.freeze({ structures: ["lattice_filaments", "interlocked_arcs"], motions: ["depth_precession", "convergent_flow"] }),
  prism_vault: Object.freeze({ structures: ["faceted_arcs", "orbital_rings"], motions: ["torsion_orbit", "braided_rotation"] }),
  neural_constellation: Object.freeze({ structures: ["connected_nodes", "woven_paths"], motions: ["parallax_drift", "convergent_flow"] }),
});
...
function verificationSource(){
  ...
  return ["qr","link","profile","share"].includes(value)?value:"direct";
}
function verificationAuthHeaders(){
  try{
    const session=JSON.parse(localStorage.getItem("trace_social_session_v1")||"null");
    return session?.access_token?{"content-type":"application/json",authorization:"Bearer "+session.access_token}:{"content-type":"application/json"};
  }catch{return {"content-type":"application/json"};}
}
function recordVerificationEvent(eventType){
  const body=JSON.stringify({proof_id:PROOF_ID,event_type:eventType,source:verificationSource()});
  const headers=verificationAuthHeaders();
  try{
    if(!headers.authorization&&navigator.sendBeacon){navigator.sendBeacon("/api/verification-events",new Blob([body],{type:"application/json"}));return;}
  }catch{}
  fetch("/api/verification-events",{method:"POST",headers,body,keepalive:true}).catch(()=>{});
}
...
async function start(){
  recordVerificationEvent(verificationSource()==="qr"?"qr_open":"public_proof_opened");
  try{
    ...
    render(data,checks);
    if(data.server_signature_valid===true) recordVerificationEvent("verification_completed");
  }catch(error){
    const seal=document.getElementById("seal");
    seal.textContent="!";
...
function normalizePeriodYear(value) {
...
async function profileStats(profile) {
  ...
  let verificationOpens = null;
  if (profile.show_activity_stats) {
    try {
      verificationOpens = await countRows("proof_verification_events", "creator_id", profile.id, (query) => query.eq("is_suspected_bot", false));
    } catch {
      verificationOpens = 0;
    }
  }
  return {
    ...
    verification_opens: verificationOpens,
  };
}
...
async function enrichWorks(works, viewerId = "") {
  ...
  for (const work of rows) {
    ...
    const proofTimestamp = Number(signedProof?.ts);
    const proofWindowSeconds = Number(signedProof?.window_s);
    const proofCreatedAt = Number.isFinite(proofTimestamp) ? new Date(proofTimestamp).toISOString() : work.created_at;
    const proofActive = Number.isFinite(proofTimestamp) && Number.isFinite(proofWindowSeconds)
      ? Date.now() >= proofTimestamp && Date.now() - proofTimestamp <= proofWindowSeconds * 1000
      : true;
    output.push({
      ...work,
      ...
      proof_status: signedProof ? (proofActive ? "active" : "expired_preserved") : "registry_unavailable",
...
app.get("/api/search", async (req, res) => {
  try {
    ...
    const [{ data: profiles, error: profileError }, { data: works, error: workError }, { data: collections, error: collectionError }] = await Promise.all([
      ...
      supabaseAdmin
        .from("collections")
        .select("id,owner_id,name,description,period_name,period_year,sort_order,featured_work_id,is_public,created_at")
        .eq("is_public", true)
...
app.get("/api/creators/:handle/works", async (req, res) => {
  try {
    ...
    const profile = await getProfileByHandle(req.params.handle, { includePrivateForUser: viewer?.id || "" });
    if (!profile) return res.status(404).json({ ok: false, error: "Creator not found" });
    const limit = Math.min(120, Math.max(1, Number.parseInt(req.query.limit || "18", 10) || 18));
    const offset = Math.max(0, Number.parseInt(req.query.offset || "0", 10) || 0);
    let query = supabaseAdmin
      .from("works")
      .select(publicWorkColumns())
...
app.get("/api/collections", async (req, res) => {
  try {
    ...
    let query = supabaseAdmin
      .from("collections")
      .select("id,owner_id,name,description,period_name,period_year,sort_order,featured_work_id,is_public,created_at,updated_at")
      .eq("owner_id", ownerId)
      .order("created_at", { ascending: false });
...
async function ownedArchiveWorkId(userId, candidate) {
  const workId = validUuid(candidate) ? String(candidate) : "";
  ...
  const { data, error } = await supabaseAdmin
    ...
    .eq("owner_id", userId)
    .maybeSingle();
  if (error) throw proofRegistryError("Archive work ownership check failed", error, 400);
  return data?.id || null;
}
...
app.post("/api/collections", socialWriteLimit, async (req, res) => {
  try {
    ...
    const featuredWorkId = await ownedArchiveWorkId(user.id, req.body?.featured_work_id);
    if (req.body?.featured_work_id && !featuredWorkId) return res.status(400).json({ ok: false, error: "Featured work must belong to this creator" });
    const { data, error } = await supabaseAdmin
      .from("collections")
      .insert({ owner_id: user.id, name, description: cleanText(req.body?.description, 500), period_name: cleanText(req.body?.period_name, 80) || null, period_year: normalizePeriodYear(req.body?.period_year), sort_order: Number.isFinite(Number(req.body?.sort_order)) ? Math.trunc(Number(req.body.sort_order)) : 0, featured_work_id: featuredWorkId, is_public: Boolean(req.body?.is_public) })
      .select("id,owner_id,name,description,period_name,period_year,sort_order,featured_work_id,is_public,created_at,updated_at")
      .single();
...
app.patch("/api/collections/:id", socialWriteLimit, async (req, res) => {
  try {
    ...
    if (Object.hasOwn(req.body || {}, "period_name")) patch.period_name = cleanText(req.body.period_name, 80) || null;
    if (Object.hasOwn(req.body || {}, "period_year")) patch.period_year = normalizePeriodYear(req.body.period_year);
    if (Object.hasOwn(req.body || {}, "sort_order")) patch.sort_order = Number.isFinite(Number(req.body.sort_order)) ? Math.trunc(Number(req.body.sort_order)) : 0;
    if (Object.hasOwn(req.body || {}, "featured_work_id")) {
      patch.featured_work_id = await ownedArchiveWorkId(user.id, req.body.featured_work_id);
      if (req.body.featured_work_id && !patch.featured_work_id) return res.status(400).json({ ok: false, error: "Featured work must belong to this creator" });
    }
...
app.post("/api/collections/:id/items", socialWriteLimit, async (req, res) => {
  try {
    ...
    const ownedWorkId = await ownedArchiveWorkId(user.id, workId);
    if (!ownedWorkId) return res.status(404).json({ ok: false, error: "Work not found in this creator archive" });
    const { error } = await supabaseAdmin.from("collection_items").upsert(
      { collection_id: collectionId, work_id: ownedWorkId, added_by: user.id, sort_order: Number.isFinite(Number(req.body?.sort_order)) ? Math.trunc(Number(req.body.sort_order)) : 0 },
...
app.get("/api/collections/:id/items", async (req, res) => {
  try {
    ...
    const { data: items, error } = await supabaseAdmin.from("collection_items").select("work_id,sort_order,created_at").eq("collection_id", id).order("sort_order", { ascending: true }).order("created_at", { ascending: true });
...
/* --------------------------------------------------------------------------
 * TRACE V51 — privacy-minimized verification activity and creator archive
 * -------------------------------------------------------------------------- */
const TRACE_EVENT_HASH_SECRET = cleanEnvValue(process.env.TRACE_EVENT_HASH_SECRET || SUPABASE_SECRET_KEY || crypto.randomBytes(32).toString("hex"));
const TRACE_EVENT_HASH_SECRET_SOURCE = process.env.TRACE_EVENT_HASH_SECRET ? "TRACE_EVENT_HASH_SECRET" : (SUPABASE_SECRET_KEY ? "derived_from_server_key" : "ephemeral_runtime");
const verificationEventLimit = makeMemoryLimiter({ windowMs: 60 * 60 * 1000, max: CONFIG.verificationEventLimitPerIp, keyPrefix: "verify-event" });
const PRODUCT_EVENT_NAMES = new Set([
  "proof_creation_started","proof_creation_completed","proof_reveal_started","proof_reveal_completed","proof_reveal_skipped",
  "artifact_downloaded","share_link_copied","story_artifact_downloaded","public_proof_opened","verification_qr_opened",
  "archive_opened","archive_work_opened","collection_created","work_added_to_collection","creator_returned_after_proof"
]);

function traceHmac(value) {
  return crypto.createHmac("sha256", TRACE_EVENT_HASH_SECRET).update(String(value || "")).digest("hex");
}
function verificationSessionHash(req, proofId = "") {
  const day = new Date().toISOString().slice(0, 10);
  return traceHmac([proofId, clientIp(req), String(req.headers["user-agent"] || ""), day].join("|"));
}
function suspectedVerificationBot(req) {
  const ua = String(req.headers["user-agent"] || "").toLowerCase();
  if (!ua) return true;
  return /(bot|crawler|spider|slurp|preview|facebookexternalhit|facebot|discordbot|slackbot|telegrambot|whatsapp|linkedinbot|pinterest|skypeuripreview|headlesschrome|lighthouse)/i.test(ua);
}
function userAgentCategory(req) {
  const ua = String(req.headers["user-agent"] || "");
  if (suspectedVerificationBot(req)) return "bot";
  if (/ipad|tablet/i.test(ua)) return "tablet";
  if (/mobi|android|iphone/i.test(ua)) return "mobile";
  return ua ? "desktop" : "unknown";
}
function referrerCategory(req) {
  const raw = String(req.headers.referer || req.headers.referrer || "").trim();
  if (!raw) return "direct";
  try {
    const ref = new URL(raw);
    const own = new URL(requestBase(req));
    if (ref.host === own.host) return "same_origin";
    if (/(google|bing|duckduckgo|yahoo|ecosia)\./i.test(ref.host)) return "search";
    if (/(instagram|facebook|tiktok|x\.com|twitter|linkedin|pinterest|reddit)\./i.test(ref.host)) return "social";
    return "external";
  } catch { return "unknown"; }
}
function coarseCountry(req) {
  const value = cleanText(req.headers["cf-ipcountry"] || req.headers["x-vercel-ip-country"], 3).toUpperCase();
  return /^[A-Z]{2}$/.test(value) ? value : null;
}
function coarseRegion(req) {
  return cleanText(req.headers["x-vercel-ip-country-region"] || "", 80) || null;
}
function verificationSourceValue(value) {
  const source = cleanText(value, 16).toLowerCase();
  return new Set(["direct","link","qr","profile","share"]).has(source) ? source : "direct";
}
async function workForProofId(proofId) {
  const { data, error } = await supabaseAdmin.from("works").select("id,owner_id,proof_id,title,is_public,hidden_from_profile").eq("proof_id", proofId).maybeSingle();
  if (error) throw proofRegistryError("Verification work lookup failed", error);
  return data || null;
}

app.post("/api/verification-events", verificationEventLimit, async (req, res) => {
  // Activity recording must never interfere with public verification.
  try {
    if (!SOCIAL_ADMIN_CONFIGURED) return res.sendStatus(204);
    const proofId = normalizeProofIdSocial(req.body?.proof_id);
    const eventType = cleanText(req.body?.event_type, 40);
    if (!proofId || !new Set(["public_proof_opened","verification_completed","qr_open"]).has(eventType)) return res.sendStatus(204);
    const row = await readProofFromSupabase(proofId).catch(() => null);
    if (!row) return res.sendStatus(204);
    const work = await workForProofId(proofId).catch(() => null);
    if (!work?.owner_id) return res.sendStatus(204);
    const viewer = await optionalSocialUser(req);
    if (viewer?.id === work.owner_id) return res.sendStatus(204);
    const sessionHash = verificationSessionHash(req, proofId);
    const threshold = new Date(Date.now() - CONFIG.verificationEventWindowMs).toISOString();
    const { data: duplicate } = await supabaseAdmin.from("proof_verification_events")
      .select("id").eq("proof_id", proofId).eq("event_type", eventType).eq("session_hash", sessionHash).gte("created_at", threshold).limit(1).maybeSingle();
    if (duplicate) return res.sendStatus(204);
    const source = verificationSourceValue(req.body?.source);
    const isBot = suspectedVerificationBot(req);
    const { error: insertError } = await supabaseAdmin.from("proof_verification_events").insert({
      proof_id: proofId, creator_id: work.owner_id, work_id: work.id, event_type: eventType,
      country_code: coarseCountry(req), coarse_region: coarseRegion(req), referrer_category: referrerCategory(req),
      source, session_hash: sessionHash, user_agent_category: userAgentCategory(req), is_suspected_bot: isBot,
    });
    if (insertError) throw proofRegistryError("Verification event insert failed", insertError);
    return res.sendStatus(204);
  } catch (error) {
    console.warn(JSON.stringify({ event: "trace_verification_event_failed", message: String(error?.message || error) }));
    return res.sendStatus(204);
  }
});

async function readCreatorActivity(creatorId, limit = 20) {
  const bounded = Math.min(50, Math.max(1, Number(limit) || 20));
  const weekStart = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString();
  const [{ data, error }, { count: total }, { count: weekly }] = await Promise.all([
    supabaseAdmin.from("proof_verification_events")
      .select("id,proof_id,work_id,event_type,created_at,country_code,coarse_region,referrer_category,source,user_agent_category,is_suspected_bot")
      .eq("creator_id", creatorId).eq("is_suspected_bot", false).order("created_at", { ascending: false }).limit(bounded),
    supabaseAdmin.from("proof_verification_events").select("*", { count: "exact", head: true }).eq("creator_id", creatorId).eq("is_suspected_bot", false),
    supabaseAdmin.from("proof_verification_events").select("*", { count: "exact", head: true }).eq("creator_id", creatorId).eq("is_suspected_bot", false).gte("created_at", weekStart),
  ]);
  if (error) throw proofRegistryError("Verification activity read failed", error);
  const workIds = [...new Set((data || []).map((event) => event.work_id).filter(validUuid))];
  let works = [];
  if (workIds.length) {
    const result = await supabaseAdmin.from("works").select("id,title").in("id", workIds);
    if (!result.error) works = result.data || [];
  }
  const workMap = new Map(works.map((work) => [work.id, work]));
  const events = (data || []).map((event) => ({
    ...event,
    work: workMap.get(event.work_id) || null,
    source_label: event.source === "qr" || event.event_type === "qr_open" ? "QR scan" : event.source === "share" ? "Shared link" : "Public proof",
  }));
  return { events, total: Number(total || 0), weekly_total: Number(weekly || 0) };
}

app.get("/api/activity", async (req, res) => {
  try {
    const { user } = await socialAuthContext(req);
    return res.json({ ok: true, activity: await readCreatorActivity(user.id, req.query.limit || 20) });
  } catch (error) { return sendSocialError(res, error); }
});

const MILESTONE_COPY = Object.freeze({
  first_proof: ["Your creator archive has begun.", "Your first creator-linked work is now part of a permanent history."],
  fifth_proof: ["Five works in your archive", "Five real Creator Proofs now form a growing body of work."],
  tenth_proof: ["Ten creator-linked works", "Your archive now contains ten documented works."],
  first_public_verification: ["Your first public verification", "Someone opened and checked one of your proof records."],
  first_qr_verification: ["Your first QR verification", "A QR scan opened one of your public proof records."],
  first_collection: ["Your first collection", "A creative period or collection now organizes part of your archive."],
  one_year_since_first_proof: ["One year of creator history", "A year has passed since your first TRACE proof."],
});
async function ensureCreatorMilestones(creatorId, works = null) {
  let workRows = works;
  if (!workRows) {
    const result = await supabaseAdmin.from("works").select("id,created_at").eq("owner_id", creatorId).order("created_at", { ascending: true });
    if (result.error) throw proofRegistryError("Milestone works lookup failed", result.error);
    workRows = result.data || [];
  }
  const candidates = [];
  if (workRows.length >= 1) candidates.push("first_proof");
  if (workRows.length >= 5) candidates.push("fifth_proof");
  if (workRows.length >= 10) candidates.push("tenth_proof");
  const firstAt = workRows[0]?.created_at ? new Date(workRows[0].created_at).getTime() : 0;
  if (firstAt && Date.now() - firstAt >= 365 * 24 * 60 * 60 * 1000) candidates.push("one_year_since_first_proof");
  const [{ count: verificationCount }, { count: qrCount }, { count: collectionCount }] = await Promise.all([
    supabaseAdmin.from("proof_verification_events").select("*", { count: "exact", head: true }).eq("creator_id", creatorId).eq("is_suspected_bot", false),
    supabaseAdmin.from("proof_verification_events").select("*", { count: "exact", head: true }).eq("creator_id", creatorId).eq("is_suspected_bot", false).or("event_type.eq.qr_open,source.eq.qr"),
    supabaseAdmin.from("collections").select("*", { count: "exact", head: true }).eq("owner_id", creatorId),
  ]);
  if (Number(verificationCount || 0) > 0) candidates.push("first_public_verification");
  if (Number(qrCount || 0) > 0) candidates.push("first_qr_verification");
  if (Number(collectionCount || 0) > 0) candidates.push("first_collection");
  for (const milestoneType of candidates) {
    await supabaseAdmin.from("creator_milestones").upsert({ creator_id: creatorId, milestone_type: milestoneType, metadata: { work_count: workRows.length } }, { onConflict: "creator_id,milestone_type", ignoreDuplicates: true });
  }
  const { data, error } = await supabaseAdmin.from("creator_milestones").select("id,milestone_type,achieved_at,dismissed_at,metadata").eq("creator_id", creatorId).is("dismissed_at", null).order("achieved_at", { ascending: false });
  if (error) throw proofRegistryError("Milestone read failed", error);
  return (data || []).map((row) => ({ ...row, title: MILESTONE_COPY[row.milestone_type]?.[0] || "Archive milestone", description: MILESTONE_COPY[row.milestone_type]?.[1] || "Your archive has reached a real milestone." }));
}

app.post("/api/milestones/:id/dismiss", socialWriteLimit, async (req, res) => {
  try {
    const { user } = await socialAuthContext(req);
    const id = String(req.params.id || "");
    if (!validUuid(id)) return res.status(400).json({ ok: false, error: "Invalid milestone ID" });
    const { data, error } = await supabaseAdmin.from("creator_milestones").update({ dismissed_at: new Date().toISOString() }).eq("id", id).eq("creator_id", user.id).select("id").maybeSingle();
    if (error) throw proofRegistryError("Milestone dismissal failed", error, 400);
    if (!data) return res.status(404).json({ ok: false, error: "Milestone not found" });
    return res.json({ ok: true });
  } catch (error) { return sendSocialError(res, error); }
});

app.get("/api/archive/:handle", async (req, res) => {
  try {
    if (!SOCIAL_ADMIN_CONFIGURED) return socialUnavailable(res, "Creator archive");
    const viewer = await optionalSocialUser(req);
    const profile = await getProfileByHandle(req.params.handle, { includePrivateForUser: viewer?.id || "" });
    if (!profile) return res.status(404).json({ ok: false, error: "Creator archive not found" });
    const isOwner = viewer?.id === profile.id;
    const limit = Math.min(120, Math.max(1, Number.parseInt(req.query.limit || "120", 10) || 120));
    let query = supabaseAdmin.from("works").select(publicWorkColumns()).eq("owner_id", profile.id).order("created_at", { ascending: false }).limit(limit);
    if (!isOwner) query = query.eq("is_public", true).eq("hidden_from_profile", false);
    const { data: rawWorks, error: workError } = await query;
    if (workError) throw proofRegistryError("Creator archive works failed", workError);
    const works = await enrichWorks(rawWorks || [], viewer?.id || "");
    let collectionQuery = supabaseAdmin.from("collections").select("id,owner_id,name,description,period_name,period_year,sort_order,featured_work_id,is_public,created_at,updated_at").eq("owner_id", profile.id).order("sort_order", { ascending: true }).order("created_at", { ascending: true });
    if (!isOwner) collectionQuery = collectionQuery.eq("is_public", true);
    const { data: collectionRows, error: collectionError } = await collectionQuery;
    if (collectionError) throw proofRegistryError("Creator archive collections failed", collectionError);
    const collections = [];
    for (const collection of collectionRows || []) collections.push({ ...collection, item_count: await countRows("collection_items", "collection_id", collection.id) });
    const sortedAsc = [...works].sort((a, b) => new Date(a.proof_created_at || a.created_at) - new Date(b.proof_created_at || b.created_at));
    const activity = isOwner ? await readCreatorActivity(profile.id, 20) : null;
    let verificationEvents = null;
    if (isOwner || profile.show_activity_stats) {
      try { verificationEvents = await countRows("proof_verification_events", "creator_id", profile.id, (q) => q.eq("is_suspected_bot", false)); } catch { verificationEvents = 0; }
    }
    const summary = {
      first_proof_date: sortedAsc[0]?.proof_created_at || sortedAsc[0]?.created_at || null,
      latest_proof_date: sortedAsc.at(-1)?.proof_created_at || sortedAsc.at(-1)?.created_at || null,
      active_proofs: works.filter((work) => work.proof_status === "active").length,
      total_historical_proofs: works.length,
      verification_events: verificationEvents,
    };
    const milestones = isOwner ? await ensureCreatorMilestones(profile.id, works.map((work) => ({ id: work.id, created_at: work.proof_created_at || work.created_at }))) : [];
    const [creator] = await enrichProfiles([profile], viewer?.id || "");
    return res.json({ ok: true, archive: { creator, works, collections, summary, activity, milestones, is_owner: isOwner } });
  } catch (error) { return sendSocialError(res, error); }
});

app.post("/api/product-events", async (req, res) => {
  try {
    if (!SOCIAL_ADMIN_CONFIGURED) return res.sendStatus(204);
    const eventName = cleanText(req.body?.event_name || req.body?.name, 64);
    if (!PRODUCT_EVENT_NAMES.has(eventName)) return res.sendStatus(204);
    const viewer = await optionalSocialUser(req);
    const proofId = normalizeProofIdSocial(req.body?.proof_id);
    const workId = validUuid(req.body?.work_id) ? req.body.work_id : null;
    await supabaseAdmin.from("trace_product_events").insert({
      user_id: viewer?.id || null, event_name: eventName, proof_id: proofId || null, work_id: workId,
      surface: cleanText(req.body?.surface, 48) || null, format: cleanText(req.body?.format, 32) || null,
      reduced_motion: typeof req.body?.reduced_motion === "boolean" ? req.body.reduced_motion : null,
      session_hash: traceHmac([viewer?.id || "guest", clientIp(req), new Date().toISOString().slice(0, 10)].join("|")),
    });
    return res.sendStatus(204);
...
app.get("/health", (req, res) => {
  ...
  return res.json({
    ok: true,
    build: "trace-v51-creator-ritual-archive-activity",
    ts: Date.now(),
    frontend_found: fs.existsSync(indexPath),
    glyph_engine_found: fs.existsSync(glyphEnginePath),
    ritual_engine_found: fs.existsSync(ritualEnginePath),
    ritual_styles_found: fs.existsSync(ritualStylesPath),
    verification_event_hash_secret_source: TRACE_EVENT_HASH_SECRET_SOURCE,
