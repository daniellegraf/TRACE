(function traceGlyphV1Factory(root){
  ...
  function normalizeGlyphSpecification(candidate){
    ...
    return Object.freeze({
      version:VERSION,
      renderer_version:candidate.renderer_version===RENDERER_V2?RENDERER_V2:RENDERER_V1,
      style,
      structure,
  ...
  function renderGlyphFromSpecification(candidate,context={}){
    ...
    const stroke=v2?strokeBase*0.92:strokeBase;
    const specEncoded=encodeURIComponent(JSON.stringify(spec));
    let svg=`<svg class="glyph3d trace-glyph-v1${v2?" trace-glyph-renderer-v2":""}" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${width} ${height}" width="${width}" height="${height}" role="img" aria-label="Deterministic TRACE proof glyph" preserveAspectRatio="xMidYMid meet" data-glyph-spec-version="${VERSION}" data-glyph-renderer-version="${spec.renderer_version}" data-glyph-spec="${specEncoded}" data-motion-model="${spec.motion}" data-rendered-primary-count="${spec.primary_path_count}" data-rendered-layer-count="${spec.layer_count}">`;
    svg+=`<defs>
      <radialGradient id="${uid}_core" cx="50%" cy="50%" r="72%"><stop offset="0" stop-color="${colors[2]}" stop-opacity="${v2?".22":".18"}"/><stop offset=".52" stop-color="${colors[0]}" stop-opacity="${v2?".08":".06"}"/><stop offset="1" stop-color="#000" stop-opacity="1"/></radialGradient>
      ...
      ${v2?`<linearGradient id="${uid}_line2" x1="1" y1="0" x2="0" y2="1"><stop offset="0" stop-color="${colors[3]}"/><stop offset=".34" stop-color="${colors[2]}"/><stop offset=".68" stop-color="${colors[1]}"/><stop offset="1" stop-color="${colors[0]}"/></linearGradient>
    ...
    if(spec.layer_count>=2){
      ...
      primary.forEach((d,i)=>{svg+=`<path class="trace-glyph-halo" data-path-index="${i}" d="${d}" fill="none" stroke="${colors[i%colors.length]}" stroke-width="${(stroke*(v2?2.6:3.2)).toFixed(2)}" stroke-linecap="round" stroke-linejoin="round" opacity="${v2?".065":".075"}"/>`;});
      if(v2){
    ...
    primary.forEach((d,i)=>{
      svg+=`<path class="trace-glyph-primary strand" data-path-index="${i}" d="${d}" fill="none" stroke="url(#${uid}_${v2&&i%2?"line2":"line"})" stroke-width="${stroke.toFixed(2)}" stroke-linecap="round" stroke-linejoin="round" opacity="${(0.62+0.34*(i+1)/spec.primary_path_count).toFixed(3)}"${v2?' vector-effect="non-scaling-stroke"':''}/>`;
    });
    if(v2){
    ...
    if(spec.layer_count>=3){
      ...
      svg+=`<g data-glyph-layer="detail" class="trace-glyph-layer trace-glyph-detail-layer" opacity="${v2?".58":".42"}">`;
      svg+=`<circle cx="${cx}" cy="${cy}" r="${r.toFixed(2)}" fill="none" stroke="${colors[2]}" stroke-width="${(stroke*0.55).toFixed(2)}" stroke-dasharray="${v2?"1.4 3.2":"2 4"}"/><circle cx="${cx}" cy="${cy}" r="${(r*0.35).toFixed(2)}" fill="${colors[1]}" opacity="${v2?".30":".22"}"/>`;
      if(v2){
    ...
    if(spec.layer_count>=4){
      ...
      const nodeCount=v2?Math.max(6,spec.symmetry*3):spec.symmetry*2;
      for(let i=0;i<nodeCount;i++){
        const a=(i/nodeCount)*Math.PI*2+(v2?(b(28)/255)*.45:0);
        const rr=min*(v2?(.28+(i%2)*.07):.34);
        svg+=`<circle cx="${(cx+Math.cos(a)*rr).toFixed(2)}" cy="${(cy+Math.sin(a)*rr).toFixed(2)}" r="${v2?(i%3===0?1.35:.82):1.15}" fill="${colors[i%colors.length]}" opacity="${v2?".60":".52"}"/>`;
        if(v2&&i%2===0){
  ...
  function startGlyphMotion(svg){
    ...
    const tick=(now)=>{
      ...
      if(spec.renderer_version===RENDERER_V2){
  ...
  const api=Object.freeze({
    VERSION,RENDERER_V1,RENDERER_V2,STYLES,normalizeGlyphSpecification,createGlyphSpecification,describeGlyphSpecification,
