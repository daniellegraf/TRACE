(function traceGlyphV1Factory(root){
  ...
  function renderGlyphFromSpecification(candidate,context={}){
    ...
    if(!spec) return "";
    const mode=clean(context.mode||"badge");
    const width=Math.max(48,Math.round(Number(context.width)||(mode==="avatar"?96:mode==="public"?160:142)));
    const height=Math.max(48,Math.round(Number(context.height)||(mode==="avatar"?96:mode==="public"?160:92)));
    const colors=palette(spec);
    const uid=`tg_${spec.geometry_seed.slice(0,10)}_${mode}`.replace(/[^a-z0-9_-]/gi,"");
    const stroke={fine:0.72,balanced:0.94,bold:1.18}[spec.stroke_profile];
    const specEncoded=encodeURIComponent(JSON.stringify(spec));
    let svg=`<svg class="glyph3d trace-glyph-v1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${width} ${height}" width="${width}" height="${height}" role="img" aria-label="Deterministic TRACE proof glyph" preserveAspectRatio="xMidYMid meet" data-glyph-spec-version="${VERSION}" data-glyph-spec="${specEncoded}" data-motion-model="${spec.motion}" data-rendered-primary-count="${spec.primary_path_count}" data-rendered-layer-count="${spec.layer_count}">`;
    svg+=`<defs><radialGradient id="${uid}_core" cx="50%" cy="50%" r="72%"><stop offset="0" stop-color="${colors[2]}" stop-opacity=".18"/><stop offset=".52" stop-color="${colors[0]}" stop-opacity=".06"/><stop offset="1" stop-color="#000" stop-opacity="1"/></radialGradient><linearGradient id="${uid}_line" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="${colors[0]}"/><stop offset=".34" stop-color="${colors[1]}"/><stop offset=".68" stop-color="${colors[2]}"/><stop offset="1" stop-color="${colors[3]}"/></linearGradient></defs>`;
    ...
    if(spec.layer_count>=2){
      ...
      primary.forEach((d,i)=>{svg+=`<path class="trace-glyph-halo" data-path-index="${i}" d="${d}" fill="none" stroke="${colors[i%colors.length]}" stroke-width="${(stroke*3.2).toFixed(2)}" stroke-linecap="round" stroke-linejoin="round" opacity=".075"/>`;});
      svg+=`</g>`;
    }
    svg+=`<g data-glyph-layer="primary" class="trace-glyph-layer trace-glyph-primary-layer" data-active-motion="${spec.motion}">`;
    primary.forEach((d,i)=>{svg+=`<path class="trace-glyph-primary strand" data-path-index="${i}" d="${d}" fill="none" stroke="url(#${uid}_line)" stroke-width="${stroke.toFixed(2)}" stroke-linecap="round" stroke-linejoin="round" opacity="${(0.62+0.34*(i+1)/spec.primary_path_count).toFixed(3)}"/>`;});
    svg+=`</g>`;
    if(spec.layer_count>=3){
      const cx=width/2,cy=height/2,r=Math.min(width,height)*(0.08+spec.density*0.04);
      svg+=`<g data-glyph-layer="detail" class="trace-glyph-layer trace-glyph-detail-layer" opacity=".42"><circle cx="${cx}" cy="${cy}" r="${r.toFixed(2)}" fill="none" stroke="${colors[2]}" stroke-width="${(stroke*0.55).toFixed(2)}" stroke-dasharray="2 4"/><circle cx="${cx}" cy="${cy}" r="${(r*0.35).toFixed(2)}" fill="${colors[1]}" opacity=".22"/></g>`;
    }
    if(spec.layer_count>=4){
      svg+=`<g data-glyph-layer="nodes" class="trace-glyph-layer trace-glyph-node-layer">`;
      for(let i=0;i<spec.symmetry*2;i++){
        const a=(i/(spec.symmetry*2))*Math.PI*2;
        const rr=Math.min(width,height)*0.34;
        svg+=`<circle cx="${(width/2+Math.cos(a)*rr).toFixed(2)}" cy="${(height/2+Math.sin(a)*rr).toFixed(2)}" r="1.15" fill="${colors[i%colors.length]}" opacity=".52"/>`;
      }
      svg+=`</g>`;
    }
    svg+=`</svg>`;
  ...
  function startGlyphMotion(svg){
    ...
    const tick=(now)=>{
      ...
      for(let i=0;i<primary.length;i++){
        const d=structuralPath(spec,i,width,height,t,{mode});
  ...
  const api=Object.freeze({
    VERSION,STYLES,normalizeGlyphSpecification,createGlyphSpecification,describeGlyphSpecification,
