...
async function verifyTraceProofCryptographically(proof) {
  ...
  const expectedCreatorId = await sha256HexText(
    `${alg}|${JSON.stringify(publicJwk)}`
  );

  if (
    String(proof.creator_id || "").toLowerCase() !==
    expectedCreatorId.toLowerCase()
  ) {
...
function supabaseHeaders(extra = {}) {
  ...
  /*
   * New sb_secret_ keys authenticate through apikey.
   * Legacy JWT service-role keys additionally use Bearer auth.
   */
...
async function readProofFromSupabase(id) {
  if (!PROOF_REGISTRY_CONFIGURED) {
    const error = new Error("Proof registry is not configured");
    error.httpStatus = 503;
    throw error;
  }

  const url =
    `${SUPABASE_URL}/rest/v1/trace_proofs` +
    `?id=eq.${encodeURIComponent(id)}` +
    `&select=id,proof,created_at&limit=1`;
  ...
  if (!response.ok) {
    const error = new Error("Proof registry read failed");
...
async function insertProofIntoSupabase(id, proof) {
  ...
  if (!response.ok) {
    ...
    error.httpStatus = response.status === 409 ? 409 : 502;
    error.registryStatus = response.status;
    throw error;
  }
...
function publicVerificationUrl(req, id) {
  return `${requestBase(req)}/verify/${encodeURIComponent(id)}`;
}
...
app.post("/proofs", proofPublishLimit, async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      String(proof.verification_url) !== expectedUrl
    ) {
      return res.status(400).json({
        ok: false,
        error: "Verification URL does not match this TRACE service",
        request_id: requestId,
      });
    }

    proof.verification_url = expectedUrl;
    ...
    if (existing) {
      const existingHash = crypto
        .createHash("sha256")
        .update(JSON.stringify(existing.proof))
        .digest("hex");
      ...
      if (existingHash !== incomingHash) {
        return res.status(409).json({
          ok: false,
          error: "A different proof already exists for this Badge ID",
          request_id: requestId,
        });
      }
...
<style>
:root{color-scheme:dark;--bg:#030507;--card:#090d13;--line:rgba(255,255,255,.11);--text:#edf6ff;--muted:rgba(237,246,255,.64);--green:#35f0a3;--red:#ff5d78;--amber:#ffd36a}
...
.ok{color:var(--green)}.badText{color:var(--red)}.warn{color:var(--amber)}
.footer{text-align:center;color:rgba(237,246,255,.42);font-size:12px;margin-top:15px}
@media(max-width:560px){.grid{grid-template-columns:1fr}.full{grid-column:auto}.card{padding:17px;border-radius:23px}}
</style>
</head>
<body>
<main>
...
async function verify(proof){
  ...
  if(alg==="Ed25519"){
    key=await crypto.subtle.importKey("jwk",proof.pub_jwk,{name:"Ed25519"},false,["verify"]);
    params={name:"Ed25519"};
  }else if(alg==="ECDSA_P256_SHA256"){
    key=await crypto.subtle.importKey("jwk",proof.pub_jwk,{name:"ECDSA",namedCurve:"P-256"},false,["verify"]);
    params={name:"ECDSA",hash:"SHA-256"};
  }else{
    throw new Error("Unsupported signature algorithm");
  }

  const sigOk=await crypto.subtle.verify(params,key,b64(proof.sig_b64),bytes);
  const creatorHash=hex(await crypto.subtle.digest("SHA-256",enc.encode(alg+"|"+JSON.stringify(proof.pub_jwk))));
  const creatorOk=creatorHash.toLowerCase()===String(proof.creator_id||"").toLowerCase();
  const now=Date.now();
  const active=Number.isFinite(Number(proof.ts))&&Number.isFinite(Number(proof.window_s))
    ? now>=Number(proof.ts)&&now-Number(proof.ts)<=Number(proof.window_s)*1000
    : false;

  return {sigOk,creatorOk,active,alg};
}
function render(data,checks){
  const proof=data.proof;
  const valid=checks.sigOk&&checks.creatorOk;
  const seal=document.getElementById("seal");
  seal.textContent=valid?"✓":"!";
  seal.className="seal "+(valid?"good":"bad");
  document.getElementById("title").textContent=valid?"Valid TRACE signature":"Invalid TRACE proof";
  document.getElementById("sub").textContent=valid
    ? (checks.active?"The signature and creator identity are valid.":"The signature is valid, but the active window has expired.")
    : "The cryptographic proof did not validate.";

  const score=Number(proof.origin?.score_0_1);
  const origin=proof.origin?.ai_flag?"AI flagged":"Human signal";
  ...
  document.getElementById("details").innerHTML=\`
    <div class="item"><span>Signature</span><b class="\${checks.sigOk?"ok":"badText"}">\${checks.sigOk?"Valid":"Invalid"}</b></div>
    ...
    <div class="item"><span>Origin</span><b>\${esc(origin)}</b></div>
    <div class="item"><span>Origin score</span><b>\${Number.isFinite(score)?(score*100).toFixed(1)+"%":"—"}</b></div>
...
async function start(){
  try{
    const response=await fetch("/proofs/"+encodeURIComponent(PROOF_ID),{headers:{accept:"application/json"}});
    const data=await response.json();
    if(!response.ok||!data.ok) throw new Error(data.error||"Proof not found");
    const checks=await verify(data.proof);
    render(data,checks);
  }catch(error){
    const seal=document.getElementById("seal");
    seal.textContent="!";
