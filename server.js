...
async function publicPresentationForProof(proofId) {
  ...
  try {
    ...
    if (work.owner_id) {
      ...
      if (profile) {
        creator = {
          ...
          glyph_style: profile.glyph_style || "spiro_flow",
    ...
    return {
      ...
      artwork_url: work.artwork_url || "",
      thumbnail_url: work.thumbnail_url || "",
      alt_text: work.alt_text || work.title || "Proof-linked artwork",
      creator
    };
  } catch (error) {
    console.warn("TRACE public proof presentation lookup failed:", error?.message || error);
    return null;
  }
}
...
app.get("/proofs/:id", async (req, res) => {
  ...
  try {
    ...
    const presentation = await publicPresentationForProof(id);
    return res.json({
      ok: true,
      id,
      proof: row.proof,
...
.sub{text-align:center;color:var(--muted);line-height:1.5;margin:0 0 18px}
.artworkStage{display:grid;gap:10px;margin:0 0 20px}.artworkFrame{width:100%;min-height:240px;max-height:62vh;display:grid;place-items:center;background:#030507;border:1px solid rgba(255,255,255,.10);border-radius:22px;overflow:hidden;box-shadow:0 20px 54px rgba(0,0,0,.30)}.artworkFrame img{display:block;width:100%;height:100%;max-height:62vh;object-fit:contain}.artworkMeta{display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap}.artworkMeta strong{font-size:15px}.creatorLink{color:var(--green);text-decoration:none;font-weight:800}.artworkFallback{padding:36px 20px;text-align:center;color:var(--muted);line-height:1.5}
...
<div id="artworkStage" class="artworkStage"><div class="artworkFrame"><div class="artworkFallback">Loading proof-linked artwork…</div></div></div>
...
function render(data,checks){
  ...
  if(artworkStage){
    const artworkUrl=String(presentation?.artwork_url||presentation?.thumbnail_url||"").trim();
    const workTitle=String(presentation?.title||proof.payload_text||proof.title||"Proof-linked artwork").trim()||"Proof-linked artwork";
    const creator=presentation?.creator||null;
    if(artworkUrl){
      const safeArtwork=esc(artworkUrl);
      const safeAlt=esc(presentation?.alt_text||workTitle);
      const creatorHtml=creator?.handle
        ? '<a class="creatorLink" href="/?creator='+encodeURIComponent(creator.handle)+'">@'+esc(creator.handle)+'</a>'
        : "";
      artworkStage.innerHTML='<div class="artworkFrame"><img src="'+safeArtwork+'" alt="'+safeAlt+'" loading="eager" decoding="async"></div><div class="artworkMeta"><strong>'+esc(workTitle)+'</strong>'+creatorHtml+'</div>';
    }else{
      artworkStage.innerHTML='<div class="artworkFrame"><div class="artworkFallback">Artwork preview unavailable for this historical proof.<br>The cryptographic proof remains verifiable.</div></div>';
    }
  }
  ...
  const artworkTitle=String(presentation?.title||proof.payload_text||proof.title||"Artwork linked by file hash").trim()||"Artwork linked by file hash";
  ...
  document.getElementById("details").innerHTML=\`
    ...
    <section class="verifySection">
      ...
      <div class="grid">
        ...
        <div class="item full"><span>Creator</span><b>\${presentation?.creator?.display_name?esc(presentation.creator.display_name)+(presentation.creator.handle?" · @"+esc(presentation.creator.handle):""):"Public creator profile unavailable"}</b></div>
...
const SOCIAL_BUILD = "trace-v51.11-qr-verify-profile-glyph-v2";
...
async function enrichWorks(works, viewerId = "") {
  ...
  const workIds = rows.map((row) => row.id).filter(Boolean);
  const proofIds = [...new Set(rows.map((row) => normalizeProofId(row.proof_id)).filter(Boolean))];

  const profilesPromise = ownerIds.length
    ? supabaseAdmin.from("profiles").select(publicProfileColumns()).in("id", ownerIds)
    : Promise.resolve({ data: [], error: null });

  const proofsPromise = proofIds.length
    ? supabaseAdmin.from("trace_proofs").select("id, proof").in("id", proofIds)
    : Promise.resolve({ data: [], error: null });

  const appreciationsPromise = workIds.length
    ? supabaseAdmin.from("appreciations").select("work_id").in("work_id", workIds)
    : Promise.resolve({ data: [], error: null });

  const savesPromise = workIds.length
    ? supabaseAdmin.from("saved_works").select("work_id").in("work_id", workIds)
    : Promise.resolve({ data: [], error: null });

  const commentsPromise = workIds.length
    ? supabaseAdmin.from("comments").select("work_id").in("work_id", workIds).eq("hidden", false)
    : Promise.resolve({ data: [], error: null });

  const viewerAppreciationsPromise = viewerId && workIds.length
    ? supabaseAdmin.from("appreciations").select("work_id").eq("user_id", viewerId).in("work_id", workIds)
    : Promise.resolve({ data: [], error: null });

  const viewerSavesPromise = viewerId && workIds.length
    ? supabaseAdmin.from("saved_works").select("work_id").eq("user_id", viewerId).in("work_id", workIds)
    : Promise.resolve({ data: [], error: null });

  const blockedPromise = blockedProfileIds(viewerId);
  ...
  const [
    ...
    savesResult,
    commentsResult,
    viewerAppreciationsResult,
    viewerSavesResult,
    blocked
  ] = await Promise.all([
    profilesPromise,
    proofsPromise,
    appreciationsPromise,
    savesPromise,
    commentsPromise,
    viewerAppreciationsPromise,
    viewerSavesPromise,
    blockedPromise
  ]);

  for (const [label, result] of [
    ["Work creator lookup", profilesResult],
    ["Work proof lookup", proofsResult],
    ["Appreciation counts", appreciationsResult],
    ["Save counts", savesResult],
    ["Comment counts", commentsResult],
    ["Viewer appreciations", viewerAppreciationsResult],
    ["Viewer saves", viewerSavesResult]
  ]) {
    if (result?.error) throw proofRegistryError(`${label} failed`, result.error);
  }

  const profileMap = new Map((profilesResult.data || []).map((profile) => [profile.id, profile]));
  const proofMap = new Map((proofsResult.data || []).map((row) => [normalizeProofId(row.id), row.proof || null]));

  const countMap = (items = []) => {
    const map = new Map();
    for (const row of items) map.set(row.work_id, (map.get(row.work_id) || 0) + 1);
    return map;
  };
  const appreciationCounts = countMap(appreciationsResult.data);
  const saveCounts = countMap(savesResult.data);
  const commentCounts = countMap(commentsResult.data);
  const viewerAppreciated = new Set((viewerAppreciationsResult.data || []).map((row) => row.work_id));
  const viewerSaved = new Set((viewerSavesResult.data || []).map((row) => row.work_id));
  ...
  for (const work of rows) {
    ...
    output.push({
      ...
      glyph_renderer_version: signedProof?.glyph_renderer_version || signedProof?.glyph_spec?.renderer_version || null,
      creator: profileMap.get(work.owner_id) || null,
      stats: {
        appreciations: appreciationCounts.get(work.id) || 0,
        saves: saveCounts.get(work.id) || 0,
        comments: commentCounts.get(work.id) || 0
      },
      viewer: {
        appreciated: viewerAppreciated.has(work.id),
        saved: viewerSaved.has(work.id),
        is_owner: viewerId === work.owner_id
      }
    });
  }
  return output;
...
app.get("/health", (req, res) => {
  ...
  return res.json({
    ok: true,
    build: "trace-v51.11-qr-verify-profile-glyph-v2",
    ts: Date.now(),
