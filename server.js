...
async function resilientSupabaseFetch(input, init = {}) {
  // Native HTTPS with family:4 avoids the opaque Node/undici "fetch failed"
  // failure observed on Render while preserving the standard Response API.
  try {
    ...
    try {
      ...
      const cause = nativeError?.code || fetchError?.cause?.code || fetchError?.code || "NETWORK_ERROR";
...
app.use((req, res, next) => {
  res.setHeader("X-TRACE-Build", "trace-v47-supabase-connectivity-fix");
  next();
});
...
app.post("/proofs", proofPublishLimit, async (req, res) => {
  ...
  try {
    ...
    const originScore = Number(origin?.score_0_1);
    const validVisualClasses = new Set(["human", "mixed", "ai_like"]);
    const originAnalysisComplete = Boolean(
      origin &&
      origin.ok === true &&
      origin.local_fallback !== true &&
      Number.isFinite(originScore) &&
      originScore >= 0 &&
      originScore <= 1 &&
      validVisualClasses.has(String(origin.visual_class || ""))
    );

    if (requiresOriginAnalysis && !originAnalysisComplete) {
      return res.status(422).json({
        ok: false,
        error: "A signed artwork proof requires a completed visual analysis",
        reason: "origin_analysis_required",
        request_id: requestId,
      });
    }
...
const SOCIAL_BUILD = "trace-v47-supabase-connectivity-fix";
...
const PUBLIC_SUPABASE_KEY_SOURCES = new Set([
  "SUPABASE_ANON_KEY",
  "NEXT_PUBLIC_SUPABASE_ANON_KEY",
  "SUPABASE_PUBLISHABLE_KEY",
  "NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY",
]);
const SUPABASE_BROWSER_AUTH_KEY = PUBLIC_SUPABASE_KEY_SOURCES.has(SUPABASE_AUTH_KEY_SOURCE)
  ? SUPABASE_AUTH_KEY
  : "";
...
function sendSocialError(res, error) {
  ...
  const status = networkFailure ? 503 : (Number(error?.httpStatus) || 500);
  const code = networkFailure ? "SUPABASE_UNREACHABLE" : (error?.code || "SOCIAL_ACTION_FAILED");
  const safeMessage = networkFailure
    ? "TRACE could not reach the account database. The server retried over IPv4. Try again once; if it persists, verify SUPABASE_URL and the public Supabase key in Render."
    : status >= 500
      ? "The account service could not complete this action."
      : error?.message || "Request failed";
  if (status >= 500) console.error("TRACE social error:", { code, message: error?.message, cause: error?.cause?.message });
...
app.get("/api/config", (req, res) => {
  res.json({
    ...
    supabase_public_url: SUPABASE_BROWSER_AUTH_KEY ? SUPABASE_URL : "",
    supabase_public_key: SUPABASE_BROWSER_AUTH_KEY,
    browser_auth_fallback_enabled: Boolean(SUPABASE_URL && SUPABASE_BROWSER_AUTH_KEY),
    diagnostics: {
      supabase_url_source: SUPABASE_URL_SOURCE,
      auth_key_source: SUPABASE_AUTH_KEY_SOURCE,
      server_key_source: SUPABASE_KEY_SOURCE,
...
app.get("/health", (req, res) => {
  ...
  return res.json({
    ok: true,
    build: "trace-v47-supabase-connectivity-fix",
    ts: Date.now(),
