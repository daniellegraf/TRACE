...
app.use((req, res, next) => {
  res.setHeader("X-TRACE-Build", "trace-v50-origin-contract-parity-fix");
  next();
});
...
const SOCIAL_BUILD = "trace-v50-origin-contract-parity-fix";
...
app.get("/health", (req, res) => {
  ...
  return res.json({
    ok: true,
    build: "trace-v50-origin-contract-parity-fix",
    ts: Date.now(),
...
async function originAnalyzeHandler(req, res) {
  ...
  try {
    ...
    const clientHashRaw = String(req.body?.client_sha256 || "").trim().toLowerCase();
