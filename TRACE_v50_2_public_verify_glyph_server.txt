...
app.use((req, res, next) => {
  res.setHeader("X-TRACE-Build", "trace-v50.1-origin-hash-integrity-fix");
  next();
});
...
function renderBadgeGlyph(proof){
  ...
  if(!unit||!host||!proof?.glyph_seed){
    if(unit) unit.hidden=true;
    return;
  }

  const ai=Number(proof?.origin?.ai_probability??proof?.origin?.score_0_1);
  const aiLike=String(proof?.origin?.classification||proof?.origin?.visual_class||"")==="ai_like"
    || (Number.isFinite(ai)&&ai>=.65);
  const imageHint=traceImageLengthHint(proof);

  host.innerHTML=makeHelixSvg(
    proof.glyph_seed,
    aiLike,
    Number.isFinite(ai)?ai:NaN,
    imageHint,
    {
      mode:"badge",
      style:String(proof.glyph_style||"spiro_flow"),
      meta:{
        payloadLen:String(proof.payload_text||"").length,
        hasImg:Boolean(proof.img_hash)
      }
    }
  );
...
app.get("/health", (req, res) => {
  ...
  return res.json({
    ok: true,
    build: "trace-v50.2-public-verify-glyph",
    ts: Date.now(),
...
if (ORIGIN_TEST_MODE) {
  try {
    const report = runOriginNormalizationTests();
