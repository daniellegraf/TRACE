...
app.post("/proofs", proofPublishLimit, async (req, res) => {
  ...
  try {
    ...
    if (!verificationUrlHasCorrectPath(
      ...
      return res.status(400).json({
        ok: false,
        error: "Verification URL path does not match this Badge ID",
        request_id: requestId,
      });
    }

    /*
     * Do not overwrite proof.verification_url here.
     * It is part of the signed payload. The registry returns its own canonical
     * public URL separately without changing the cryptographic proof.
     */
