...
app.use((req, res, next) => {
  res.setHeader("X-TRACE-Build", "trace-v33-social-creator-network");
  next();
});
...
const SUPABASE_AUTH_KEY = supabaseAuthKeyEnv.value;
const SUPABASE_AUTH_KEY_SOURCE = supabaseAuthKeyEnv.source;
...
const SOCIAL_BUILD = "trace-v33-social-mvp";
...
function socialUnavailable(res, feature = "Social creator network") {
  return res.status(503).json({
    ...
    requires: ["SUPABASE_URL", "SUPABASE_SECRET_KEY", "SUPABASE_ANON_KEY"],
  });
}
...
app.post("/api/auth/signup", socialAuthLimit, async (req, res) => {
  try {
    if (!SOCIAL_AUTH_CONFIGURED || !SOCIAL_ADMIN_CONFIGURED) return socialUnavailable(res, "Account creation");
    ...
    if (!validHandle(handle)) {
      return res.status(400).json({ ok: false, error: "Handle must be 3–32 characters using letters, numbers, dots, underscores or hyphens" });
    }
    if (password.length < 8 || password.length > 128) {
      return res.status(400).json({ ok: false, error: "Password must be 8–128 characters" });
    }

    const existing = await getProfileByHandle(handle, { includePrivateForUser: "__none__" });
    ...
    if (createError || !created?.user) {
      const error = new Error(createError?.message || "Account could not be created");
...
app.post("/api/auth/login", socialAuthLimit, async (req, res) => {
  try {
    ...
    const email = cleanText(req.body?.email, 254).toLowerCase();
    const password = String(req.body?.password || "");
    if (!validEmail(email) || !password) return res.status(400).json({ ok: false, error: "Email and password are required" });
    ...
    if (!profile) {
      const missing = new Error("This account has no TRACE creator profile");
...
app.get("/api/auth/me", async (req, res) => {
  try {
    ...
    const stats = profile ? await profileStats(profile) : null;
...
app.post("/api/profile/link-creator", socialWriteLimit, async (req, res) => {
  try {
    ...
    if (profile.creator_id && profile.creator_id !== creatorId) {
      return res.status(409).json({ ok: false, error: "This social profile is already linked to a different Creator ID" });
    }
    const { data, error } = await supabaseAdmin
      .from("profiles")
...
app.get("/health", (req, res) => {
  ...
  return res.json({
    ok: true,
    build: "trace-v33-social-creator-network",
    ts: Date.now(),
