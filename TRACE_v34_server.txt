...
app.use((req, res, next) => {
  res.setHeader("X-TRACE-Build", "trace-v34-simple-social-onboarding");
  next();
});
...
const SOCIAL_BUILD = "trace-v34-social-profile-first";
...
app.post("/api/auth/signup", socialAuthLimit, async (req, res) => {
  try {
    ...
    if (signUpError || !signedUp?.user) {
      const error = new Error(signUpError?.message || "Account could not be created");
...
app.get("/health", (req, res) => {
  ...
  return res.json({
    ok: true,
    build: "trace-v34-simple-social-onboarding",
    ts: Date.now(),
